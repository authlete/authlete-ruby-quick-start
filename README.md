...
5. Otherwise, if the response from the authorization endpoint of the SNS contains
   ...
   - Access the token endpoint of the SNS with the authorization code (= the value
     of `code` parameter) to get issued an access token. For example, in the case
...
of the client application. If you have not changed the configuration, the redirection
...
2.0 authorization flow. What makes this possible is the architecture of OpenID
...
* **Data Format**
    ...
    - `password`
       ...
       Otherwise, when `sns` is not `null`, `password` has no meaning.
...
## 2.9 About Sample Implementations
